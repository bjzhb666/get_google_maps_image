...
// Given one center GPS, we return a pic with given zoom level, given API key, given size centered at the GPS.
const ZOOM = 20
...
func main() {
	...
	for _, region := range regions {
		for x := -region.RadiusX; x < 0; x++ {
			for y := -region.RadiusY; y < 0; y++ {
				fname := fmt.Sprintf("%s/%s_%d_%d_sat.png", outDir, region.Name, x, y)
				// 如果fname文件存在，os.Stat(fname)将返回该文件的信息和nil错误。
				// 如果文件不存在或发生其他错误，os.Stat(fname)将返回nil和描述错误的error对象。
	...
	for _, tile := range requiredTiles {
		...
		for xOffset := 0; xOffset < 4096; xOffset += 512 {
			for yOffset := 0; yOffset < 4096; yOffset += 512 {
				centerWorld := tile.Region.CenterWorld.Add(common.Point{float64(tile.X*4096/2 + xOffset + 256),
					float64(-(tile.Y*4096/2 + yOffset + 256))}.Scale(widthWorld))
				// fmt.Println("centerWorld", centerWorld)
				// fmt.Println((tile.X*4096/2 + xOffset + 256), -(tile.Y*4096/2 + yOffset + 256)) // -4096, 4096
